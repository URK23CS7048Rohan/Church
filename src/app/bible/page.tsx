...
import { NativeBible } from "@/components/bible/NativeBible";
...
function WebBible() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!match) throw new Error("Could not parse reference. Please use format like 'John 3:16'");
      ...
      if (!matchedBook) throw new Error(`Could not find book matching "${match[1]}".`);
      ...
      const formattedVerses = helloAOVerses
        ...
        .map((node: any) => {
          const text = node.content
            ...
            .join("");
          return { book_id: bookCode, book_name: matchedBook, chapter, verse: node.number, text };
        });
        
      if (formattedVerses.length === 0) throw new Error(`Verse ${verseNum} not found.`);
      ...
      <main className="min-h-screen pt-32 pb-20 lg:pb-0 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-6">
            <SectionHeader eyebrow={t("nav_bible")} title={t("bible_title")} titleHighlight={t("bible_title_highlight")} subtitle={t("bible_subtitle")} />
            <p className="font-body text-fog italic text-sm mt-3 max-w-2xl mx-auto">"{t("bible_scripture")}"</p>
            <p className="font-accent text-xs text-sacred/70 tracking-widest mt-1">{t("bible_scripture_ref")}</p>
          </div>
            ...
            <div className="lg:col-span-1 space-y-6">
              <GlassCard className="p-5 space-y-5 border border-white/5 bg-surface-1">
                ...
                <div>
                  ...
                  <select value={selectedTranslation} onChange={(e) => setSelectedTranslation(e.target.value)} className="w-full glass bg-midnight text-ivory px-3 py-2 rounded-xl text-sm font-body border border-white/10 focus:border-sacred outline-none">
                    {TRANSLATIONS.map((t) => <option key={t.id} value={t.id} className="bg-surface-2">{t.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-label text-fog uppercase tracking-wider mb-2">{t("bible_book_label")}</label>
                  <select value={selectedBook} onChange={(e) => { setSelectedBook(e.target.value); setSelectedChapter(1); }} className="w-full glass bg-midnight text-ivory px-3 py-2 rounded-xl text-sm font-body border border-white/10 focus:border-sacred outline-none">
                    {BOOKS_OF_BIBLE.map((b) => <option key={b} value={b} className="bg-surface-2">{b}</option>)}
                  </select>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-1.5 max-h-48 overflow-y-auto pr-1">
                    {Array.from({ length: totalChapters }, (_, i) => i + 1).map((ch) => (
                      <button key={ch} onClick={() => setSelectedChapter(ch)} className={`py-1.5 text-xs font-label rounded-lg transition-all border ${selectedChapter === ch ? "bg-gradient-gold text-midnight border-transparent font-bold" : "glass hover:border-sacred/40 text-fog hover:text-ivory"}`}>
                        {ch}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-1">
                    {[{ id: "text-sm", name: "SM" }, { id: "text-base", name: "MD" }, { id: "text-lg", name: "LG" }, { id: "text-xl", name: "XL" }].map((sz) => (
                      <button key={sz.id} onClick={() => setFontSize(sz.id)} className={`py-1 text-xs font-label rounded-lg transition-all ${fontSize === sz.id ? "bg-sacred/20 border border-sacred text-sacred font-bold" : "glass border border-white/5 hover:border-white/20 text-fog"}`}>
                        {sz.name}
                      </button>
                    ))}
                  </div>
              ...
              <GlassCard className="p-5 space-y-4 border border-white/5 bg-surface-1">
                ...
                <form onSubmit={handleSearch} className="space-y-2">
                  <input type="text" placeholder={t("bible_lookup_placeholder")} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full glass bg-midnight text-ivory px-3 py-2.5 rounded-xl text-sm font-body border border-white/10 outline-none focus:border-sacred" />
                  <Button type="submit" size="sm" className="w-full">{searching ? t("bible_searching") : t("bible_lookup_btn")}</Button>
                </form>
                {searchError && (
                  ...
                  <div className="mt-4 border-t border-white/5 pt-4 space-y-3 max-h-60 overflow-y-auto">
                    ...
                    {searchResults.map((result, idx) => (
                      <div key={idx} className="text-xs bg-surface-2/40 p-2.5 rounded-xl border border-white/5">
                        <span className="font-bold text-sacred">{result.book_name} {result.chapter}:{result.verse}</span>
              ...
              <GlassCard className="p-8 md:p-12 border border-white/10 bg-surface-1 shadow-[0_12px_48px_rgba(0,0,0,0.5)] min-h-[500px] flex flex-col">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/5 pb-5 mb-8 gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="font-heading text-2xl font-bold text-ivory">{selectedBook} {selectedChapter}</h2>
                      <span className="font-label text-xs text-fog uppercase tracking-wider">{TRANSLATIONS.find((t) => t.id === selectedTranslation)?.name}</span>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={handlePrevChapter} className="w-10 h-10 rounded-xl glass flex items-center justify-center text-fog hover:text-ivory hover:border-sacred/40 transition-colors" title="Previous Chapter"><ChevronLeft size={18} /></button>
                    <button onClick={handleNextChapter} className="w-10 h-10 rounded-xl glass flex items-center justify-center text-fog hover:text-ivory hover:border-sacred/40 transition-colors" title="Next Chapter"><ChevronRight size={18} /></button>
                  </div>
                </div>
                ...
                <div className="flex-grow">
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center p-6 bg-ember/5 rounded-2xl border border-ember/10">
                      ...
                      <Button onClick={() => fetchChapter(selectedBook, selectedChapter, selectedTranslation)} size="sm">{t("bible_retry")}</Button>
                    </div>
                  ) : (
                    <div className={cn(fontSize, "font-display leading-relaxed text-ivory/90 space-y-4 max-w-3xl mx-auto", isRtlTranslation ? "text-right" : "text-left")} dir={isRtlTranslation ? "rtl" : "ltr"}>
                      {verses.map((v) => (
                        <span key={v.verse} className={cn("inline group relative", isRtlTranslation ? "ml-2" : "mr-2")}>
                          <sup className="font-label text-[10px] font-bold text-sacred mr-1 select-none">{v.verse}</sup>
                          <span className="hover:text-ivory transition-colors duration-150 inline">{v.text}</span>
                        </span>
                      ))}
                    </div>
                  )}
                ...
                <div className="border-t border-white/5 pt-6 mt-12 flex justify-between items-center text-xs font-label text-fog">
                  ...
                  <div className="flex gap-4">
                    <button onClick={handlePrevChapter} className="hover:text-sacred transition-colors flex items-center gap-1"><ChevronLeft size={14} /> {t("bible_prev")}</button>
                    <button onClick={handleNextChapter} className="hover:text-sacred transition-colors flex items-center gap-1">{t("bible_next")} <ChevronRight size={14} /></button>
                  </div>
                </div>
              </GlassCard>
            </div>
